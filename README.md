...
- `arm` 默认写入 `l4t`、`ARM_without_cuda`、`ARM_with_cuda`、`thor_spark`、`SOPHON_bm1688`

脚本只查找精确表头 `agent-room`。表头不存在时只新增一列并写入表头/仓库地址，不会反复扩出空列。

## Agent Room VOS 发布

VOS app 包发布走 `ictrek.app/scripts/update_version.sh patch|minor|major`。该脚本只更新 `ictrek.app/VERSION`、提交、打 `vos-agent-room-v${VERSION}` tag 并 push；本地不会读取飞书，也不会生成 release tar。

GitHub Actions 收到 tag 后读取仓库 secrets 中的 Feishu 凭证，查询 `agent-room` 最新镜像 tag，生成 `agent-room_${VERSION}_pull.tar` 并上传到标准 `v${VERSION}` release。当前不依赖其他 VOS app 的 release 版本，因此不需要 `VOS_DEPENDENCY_RELEASE_TOKEN`。
